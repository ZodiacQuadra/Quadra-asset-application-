...
import { AssetRepairRequestRecord } from "../../Services/AssetRepairRequestService";
...
type RepairFilterTab = "All" | "Pending" | "Approved" | "Unrepairable" | "Rejected";

interface AdminRepairSubScreenProps {
  requests: AssetRepairRequestRecord[];
  loading: boolean;
  onSelectRequest: (request: AssetRepairRequestRecord) => void;
  onRefresh?: () => void;
}

export const AdminRepairSubScreen: React.FC<AdminRepairSubScreenProps> = ({
  requests,
  loading,
  onSelectRequest,
}) => {
  const [filterTab, setFilterTab] = useState<RepairFilterTab>("All");
  ...
  const counts = useMemo(() => {
    return {
      All: requests.length,
      Pending: requests.filter((r) => r.RequestStatus === "Pending" || r.RequestStatus === "Re-Progress").length,
      Approved: requests.filter((r) => r.RequestStatus === "Approved").length,
      Unrepairable: requests.filter((r) => r.RequestStatus === "Unrepairable").length,
  ...
  const filtered = useMemo(() => {
    return requests.filter((r) => {
      const isPending = r.RequestStatus === "Pending" || r.RequestStatus === "Re-Progress";
      const isApproved = r.RequestStatus === "Approved";
      const isUnrepairable = r.RequestStatus === "Unrepairable";
      const isRejected = r.RequestStatus === "Rejected";

      if (filterTab === "Pending" && !isPending) return false;
      if (filterTab === "Approved" && !isApproved) return false;
      if (filterTab === "Unrepairable" && !isUnrepairable) return false;
      ...
      if (search.trim()) {
        ...
        const matches =
          ...
          (r.AssetTagID || "").toLowerCase().includes(term) ||
          (r.IssueType || "").toLowerCase().includes(term) ||
          (r.ProblemCategory || "").toLowerCase().includes(term) ||
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      >
        ...
        >
          {[
            ...
            { id: "Pending", label: "Pending Diagnostics", count: counts.Pending },
            { id: "Approved", label: "In Repair / Fixed", count: counts.Approved },
            { id: "Unrepairable", label: "Unrepairable", count: counts.Unrepairable },
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => {
                  setFilterTab(tab.id as RepairFilterTab);
                  setPage(1);
                }}
                style={{
                  border: "none",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
          <div style={{ position: "relative", minWidth: "280px" }}>
            <Input
              ...
              placeholder="Search repair, asset, tag, issue..."
              value={search}
              onChange={(_, d) => {
                setSearch(d.value);
                setPage(1);
      ...
      {loading ? (
        <div style={{ display: "flex", justifyContent: "center", padding: "60px" }}>
          <Spinner label="Loading repair requests..." />
        </div>
      ) : filtered.length === 0 ? (
        <div
          style={{
        ...
        >
          <Text size={300} style={{ color: "#64748B" }}>
            No repair requests found matching this filter or search query.
          </Text>
        </div>
      ) : viewMode === "cards" ? (
              ...
              >
                <div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: "13px", color: "#64748B", marginTop: "3px" }}>
                          {r.RequestedByEmail || "Staff Member"}
                        </div>
                      </div>
                    </div>
                  ...
                  >
                    ...
                    >
                      <WrenchRegular style={{ fontSize: "16px", color: "#107C10" }} />
                      <span style={{ color: "#334155", fontWeight: 600 }}>
                        {r.AssetName} ({r.AssetTagID || "TAG"})
                      </span>
                    </div>
                    <div
                      style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "10px" }}>
                    ...
                    >
                      {r.ProblemCategory || r.IssueType || "Hardware Issue"}
                    </span>
                  </div>
                  ...
                  >
                    {r.Problem || "Employee reported hardware fault requiring technical diagnostics."}
                  </div>
                </div>
                ...
                <div>
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      ...
                      <div>
                        <div style={{ fontSize: "13px", fontWeight: 600, color: "#1E293B" }}>
                          {isPending
                            ? "Awaiting Admin Diagnostics"
                            : isApproved
                            ? `Approved (${r.ApprovedAdminName || "Admin"})`
                            : isUnrepairable
                            ? "Marked Unrepairable"
                            : `Status: ${r.RequestStatus}`}
                        </div>
                        <div style={{ fontSize: "11px", color: "#64748B" }}>
                          {isUnrepairable && r.ReplacementRequestNumber
                            ? `Replacement Request: ${r.ReplacementRequestNumber}`
                    ...
                    <div>
                      ...
                      >
                        {isPending ? "Review Diagnostics" : "View Details"}
                      </button>
                    </div>
                  </div>
                </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: "13px" }}>
              <thead>
                <tr style={{ background: "#F8FAFC", borderBottom: "1px solid #E2E8F0" }}>
                  ...
                  <th style={{ padding: "14px 18px", color: "#64748B", fontWeight: 600 }}>Asset & Tag</th>
                  <th style={{ padding: "14px 18px", color: "#64748B", fontWeight: 600 }}>Issue / Category</th>
                ...
                {paged.map((r) => (
                  ...
                  >
                    ...
                    <td style={{ padding: "14px 18px", color: "#334155" }}>
                      ...
                      <div style={{ fontSize: "11px", color: "#64748B" }}>{r.AssetTagID}</div>
                    </td>
                    <td style={{ padding: "14px 18px", color: "#475569" }}>
                      {r.ProblemCategory || r.IssueType}
                    </td>
                    ...
                    <td style={{ padding: "14px 18px" }}>
                      <Badge
                        appearance="tint"
                        color={
                          r.RequestStatus === "Pending" || r.RequestStatus === "Re-Progress"
                            ? "warning"
...
export default AdminRepairSubScreen;
