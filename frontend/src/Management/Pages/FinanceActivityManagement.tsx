...
import {
  ...
  FinanceActivity as APIFinanceActivity,
  FinanceActivityStatus,
  CreateFinanceActivityData,
  UpdateFinanceActivityData,
} from "../../Services/FinanceManagement";

interface FinanceActivity
  extends Omit<
    APIFinanceActivity,
    "id" | "createdByUserId" | "modifiedByUserId"
  > {
  id: string;
  title: string;
  description: string;
...
interface FinanceActivityError {
  title?: string;
}

interface SortConfig {
  key: keyof FinanceActivity;
  direction: "asc" | "desc";
}

function FinanceActivityManagement() {
  const [financeActivities, setFinanceActivities] = useState<FinanceActivity[]>(
    []
  );
  const [errors, setErrors] = useState<{ [key: string]: FinanceActivityError }>(
    {}
  );
  ...
  useEffect(() => {
    fetchFinanceActivities();
  }, [accessToken]);

  const fetchFinanceActivities = async () => {
    setIsLoading(true);
    try {
      ...
      if (response.success && response.data) {
        const activitiesWithTimestamps = response.data.financeActivities.map(
          (activity: any) => ({
            ...activity,
            ...
            status: activity.status || ("Active" as FinanceActivityStatus),
          })
        );
        setFinanceActivities(activitiesWithTimestamps);
      } else {
        showToast(
          "error",
          "Error fetching finance activities",
          response.message
        );
      }
    } catch (error) {
      console.error("Error fetching finance activities:", error);
      showToast(
        "error",
        "Error fetching finance activities",
        "Please try again."
  ...
  const addFinanceActivity = () => {
    const newActivity: FinanceActivity = {
      ...
      status: "Draft" as FinanceActivityStatus,
    };
    // setFinanceActivities([...financeActivities, newActivity]);
    setFinanceActivities([newActivity, ...financeActivities]);
    setIsEditMode(true);
  ...
  const validateActivityTitle = (id: string, title: string): boolean => {
    if (!title.trim()) {
      setErrors((prevErrors) => ({
        ...prevErrors,
        [id]: { ...prevErrors[id], title: "Activity name cannot be empty" },
      }));
      return false;
    }
    ...
    if (specialCharRegex.test(title)) {
      setErrors((prevErrors) => ({
        ...
        [id]: {
          ...prevErrors[id],
          title: "Special characters are not allowed in the activity name",
        },
      }));
      return false;
    }

    const isDuplicate = financeActivities.some(
      (activity) =>
        activity.id !== id &&
    ...
    if (isDuplicate) {
      setErrors((prevErrors) => ({
        ...prevErrors,
        [id]: { ...prevErrors[id], title: "Activity name already exists" },
      }));
      return false;
    }
  ...
  const validateAllActivities = () => {
    let isValid = true;
    financeActivities.forEach((activity) => {
  ...
  const handleInputChange = (
    id: string,
    field: keyof FinanceActivity,
    value: string
  ) => {
    const updatedActivities = financeActivities.map((activity) =>
      activity.id === id
        ? { ...activity, [field]: value, modifiedAt: new Date().toISOString() }
        : activity
    );
    setFinanceActivities(updatedActivities);
  ...
  const saveAllActivities = async () => {
    ...
    for (const activity of financeActivities) {
      try {
        if (activity.isNew) {
          const createData: CreateFinanceActivityData = {
            title: activity.title,
            description: activity.description || "",
            status: "Active" as FinanceActivityStatus,
            createdByUserId: currentUser?.userID || "",
          };
          const response = await createFinanceActivity(createData, accessToken);
          if (!response.success) throw new Error(response.message);
        } else {
          const updateData: UpdateFinanceActivityData = {
            title: activity.title,
            description: activity.description || "",
            status: activity.status || ("Active" as FinanceActivityStatus),
            modifiedByUserId: currentUser?.userID || "",
          };
          const response = await updateFinanceActivity(
            activity.id,
            updateData,
            accessToken
          );
    ...
    if (!hasErrors) {
      showToast(
        "success",
        "Success",
        "All finance activities saved successfully."
      );
    }
    await fetchFinanceActivities();
    setIsSaving(false);
    setIsEditMode(false);
  };
  ...
  const handleDeleteActivity = async (id: string) => {
    setDeletingId(id);
    const activityToDelete = financeActivities.find(
      (activity) => activity.id === id
    );

    if (activityToDelete?.isNew) {
      setFinanceActivities((prevActivities) =>
        prevActivities.filter((activity) => activity.id !== id)
      );
      showToast("success", "Success", "New activity removed successfully.");
    } else {
      try {
        const response = await deleteFinanceActivity(
          id,
          currentUser?.userID || "",
          accessToken
        );
        if (response.success) {
          ...
          showToast(
            "success",
            "Success",
            "Finance Activity deleted successfully."
          );
        } else {
  ...
  const handleSort = (key: keyof FinanceActivity) => {
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <Subtitle2 className="text-[#063762]">
                  Finance Activity Management
                </Subtitle2>
              </div>
              <div>
                {" "}
                <Caption1 className="text-gray-600">
                  Manage finance activities for your organization
                </Caption1>
              </div>
            </div>
          </div>
            ...
            {sortedActivities.length > 0 ? (
                  ...
                  <Table sortable className="w-full">
                    ...
                    <TableBody>
                      {sortedActivities.map(
                        (activity: FinanceActivity, index: number) => (
                          <TableRow
                            key={activity.id}
                  ...
                  <div className="bg-gray-50 border-t border-gray-100 px-2 py-4">
                    <div className="flex items-center justify-between">
                      <Caption1 className="text-gray-600 font-medium">
                        Showing {sortedActivities.length} of{" "}
                        {financeActivities.length} activities
                      </Caption1>
                      {isEditMode && (
                        <Button
                          icon={
              ...
              <div className="text-center py-20 flex flex-col items-center bg-white/50 rounded-lg shadow-sm border border-white">
                <div className="w-20 h-20 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-full flex items-center justify-center mb-6">
                  <TaskListSquareAddRegular className="w-10 h-10 text-blue-600" />
                </div>
                <Subtitle2 className="mb-3 text-gray-700">
                  No finance activities found
                </Subtitle2>
                <div className="text-center">
                  <Text className="text-gray-500 max-w-md mx-auto leading-relaxed">
                    {searchTerm
                      ...
                      : "Get started by creating your first finance activity to manage your organization's financial tasks."}
                  </Text>
                </div>
                {searchTerm ? (
                  <Button
...
export default FinanceActivityManagement;
