...
import {
  ...
  updateAdminActivity,
  deleteAdminActivity,
  AdminActivity as APIAdminActivity,
  ActivityStatus,
  CreateAdminActivityData,
  UpdateAdminActivityData,
...
function AdminActivityManagement() {
  ...
  const [errors, setErrors] = useState<{ [key: string]: AdminActivityError }>(
    {}
  );
  ...
  const fetchAdminActivities = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        const activitiesWithTimestamps = response.data.adminActivities.map(
          (activity: any) => ({
            ...activity,
        ...
        setAdminActivities(activitiesWithTimestamps);
      } else {
        showToast("error", "Error fetching admin activities", response.message);
      }
    } catch (error) {
      console.error("Error fetching admin activities:", error);
      showToast(
        "error",
        "Error fetching admin activities",
        "Please try again."
      );
    } finally {
      setIsLoading(false);
  ...
  const addAdminActivity = () => {
    ...
    // setAdminActivities([...adminActivities, newActivity]);
  ...
  const validateActivityTitle = (id: string, title: string): boolean => {
    ...
    const isDuplicate = adminActivities.some(
      (activity) =>
        activity.id !== id &&
  ...
  const handleInputChange = (
    ...
    const updatedActivities = adminActivities.map((activity) =>
      activity.id === id
  ...
  const saveAllActivities = async () => {
    ...
    for (const activity of adminActivities) {
      try {
        if (activity.isNew) {
          const createData: CreateAdminActivityData = {
            title: activity.title,
            description: activity.description || "",
            status: "Active" as ActivityStatus,
            createdByUserId: currentUser?.userID || "",
          };
          const response = await createAdminActivity(createData, accessToken);
          if (!response.success) throw new Error(response.message);
        } else {
          const updateData: UpdateAdminActivityData = {
            title: activity.title,
            description: activity.description || "",
            status: activity.status || ("Active" as ActivityStatus),
            modifiedByUserId: currentUser?.userID || "",
          };
          const response = await updateAdminActivity(
            activity.id,
            updateData,
            accessToken
          );
    ...
    if (!hasErrors) {
      showToast(
        "success",
        "Success",
        "All admin activities saved successfully."
      );
    }
  ...
  const handleDeleteActivity = async (id: string) => {
    setDeletingId(id);
    const activityToDelete = adminActivities.find(
      (activity) => activity.id === id
    );

    if (activityToDelete?.isNew) {
      setAdminActivities((prevActivities) =>
        prevActivities.filter((activity) => activity.id !== id)
      );
      showToast(
        "success",
        "Success",
        "New admin activity removed successfully."
      );
    } else {
      try {
        const response = await deleteAdminActivity(
          id,
          currentUser?.userID || "",
          accessToken
        );
        if (response.success) {
          ...
          showToast(
            "success",
            "Success",
            "Admin Activity deleted successfully."
          );
        } else {
  ...
  const handleSort = (key: keyof AdminActivity) => {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-8 h-full">
        <Spinner />
        <Body1Strong className="mt-2">Loading Admin Activities...</Body1Strong>
      </div>
    );
  }
            ...
            <div>
              ...
              <div>
                ...
                <Caption1 className="text-gray-600">
                  Manage administrative activities for your organization
                </Caption1>
              </div>
            </div>
          </div>
                  ...
                  <Table sortable className="w-full">
                    ...
                    <TableBody>
                      {sortedActivities.map(
                        (activity: AdminActivity, index: number) => (
                          <TableRow
                            key={activity.id}
                  ...
                  <div className="bg-gray-50 border-t border-gray-100 px-2 py-4">
                    <div className="flex items-center justify-between">
                      <Caption1 className="text-gray-600 font-medium">
                        ...
                        {adminActivities.length} activities
                      </Caption1>
                      {isEditMode && (
                        <Button
                          icon={
                ...
                <div className="text-center">
                  <Text className="text-gray-500 max-w-md mx-auto leading-relaxed">
                    {searchTerm
                      ...
                      : "Get started by creating your first admin activity to manage your organization's administrative tasks."}
                  </Text>
                </div>
                {searchTerm ? (
                  <Button
