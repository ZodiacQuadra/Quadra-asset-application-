...
import { getAdminDashboardExits } from "../../Services/Offboarding";
...
const EmployeeExitsDashboard = () => {
  ...
  const loadExits = async () => {
    try {
      ...
      const response = await getAdminDashboardExits(accessToken);
  ...
  const calculateStats = (data: EmployeeExit[]) => {
    const stats = {
      ...
      pending: data.filter(
        (exit: EmployeeExit) =>
          exit.AdminStatus === "Pending" && exit.ITStatus === "Completed"
      ).length,
      locked: data.filter((exit: EmployeeExit) => exit.AdminStatus === "Locked")
        .length,
      completed: data.filter(
        (exit: EmployeeExit) => exit.AdminStatus === "Completed"
      ).length,
    };
  ...
  useEffect(() => {
    ...
    // Status filter
    if (statusFilter !== "all") {
      filtered = filtered.filter((exit) => exit.AdminStatus === statusFilter);
    }
  ...
  const handleViewDetails = (id: string) => {
    navigate(`/offboard/AdminClearance/${id}`);
  };
      ...
      <div className="flex justify-between items-center">
        <div>
          <Subtitle2 className="text-[#902D0F] font-bold">
            Admin Department - Employee Offboarding
          </Subtitle2>
          {/* <div>
            <Caption1 className="text-gray-600">
              Process admin clearances and documentation
            </Caption1>
          </div> */}
        </div>
      </div>
            ...
            <Table sortable className="w-full">
              ...
              <TableBody>
                {paginatedExits.map((exit) => {
                  const currentStatus = exit.AdminStatus;
                  return (
                    <TableRow
                      key={exit.ID}
                    ...
                    >
                      ...
                      <TableCell>
                        <Text size={200} className="text-gray-600 !text-xs">
                          {exit.WorkLocation}
                        </Text>
                      </TableCell>
