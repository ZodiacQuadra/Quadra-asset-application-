...
import { getITDashboardExits } from "../../Services/Offboarding";
...
const EmployeeExitsDashboard = () => {
  ...
  const loadExits = async () => {
    try {
      ...
      const response = await getITDashboardExits(accessToken);
  ...
  const calculateStats = (data: EmployeeExit[]) => {
    const stats = {
      ...
      pending: data.filter(
        (exit: EmployeeExit) =>
          exit.ITStatus === "Pending" && exit.HeadStatus === "Completed"
      ).length,
      locked: data.filter((exit: EmployeeExit) => exit.ITStatus === "Locked")
        .length,
      completed: data.filter(
        (exit: EmployeeExit) => exit.ITStatus === "Completed"
      ).length,
    };
  ...
  useEffect(() => {
    ...
    // Status filter
    if (statusFilter !== "all") {
      filtered = filtered.filter((exit) => exit.ITStatus === statusFilter);
    }
  ...
  const handleViewDetails = (id: string) => {
    navigate(`/offboard/ITClearance/${id}`);
  };
      ...
      <div className="flex justify-between items-center">
        <div>
          <Subtitle2 className="text-[#902D0F] font-bold">
            IT Department - Employee Offboarding
          </Subtitle2>
          {/* <div>
            <Caption1 className="text-gray-600">
              Handle IT asset returns and account deactivation
            </Caption1>
          </div> */}
        </div>
      </div>
            ...
            <Table sortable className="w-full">
              ...
              <TableBody>
                {paginatedExits.map((exit) => {
                  const currentStatus = exit.ITStatus;
                  return (
                    <TableRow
                      key={exit.ID}
