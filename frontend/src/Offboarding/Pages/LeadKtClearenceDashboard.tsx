...
import { getEmployeeExitsByManager, getEmployeeExitsByManagerForKt } from "../../Services/Offboarding";
...
export interface EmployeeExit {
  ...
  HeadKtStatus: string;
  HeadKtUserId: string;
  HeadKtRemainder?: string | null
  HeadKtDate?: string | null
}
...
const EmployeeExitsDashboard = () => {
  ...
  const loadExits = async () => {
    try {
      ...
      const response = await getEmployeeExitsByManagerForKt(
        currentUser.userID,
        accessToken
      );
  ...
  const calculateStats = (data: EmployeeExit[]) => {
    const stats = {
      total: data.length,
      pending: data.filter(
        (exit: EmployeeExit) => exit.HeadKtStatus === "Pending"
      ).length,
      locked: data.filter((exit: EmployeeExit) => exit.HeadKtStatus === "Locked")
        .length,
      completed: data.filter(
        (exit: EmployeeExit) => exit.HeadKtStatus === "Completed"
      ).length,
    };
  ...
  useEffect(() => {
    ...
    // Status filter
    if (statusFilter !== "all") {
      filtered = filtered.filter((exit) => exit.HeadKtStatus === statusFilter);
    }
  ...
  const handleViewDetails = (id: string) => {
    navigate(`/offboard/LeadKtClearance/${id}`);
  };
  ...
  return (
    <FluentProvider
      ...
      className="space-y-4 min-h-screen mx-auto"
    >
      {/* Header */}
      <div className="flex justify-between items-center">
        <div>
          <Subtitle2 className="text-[#902D0F] font-bold">
            My Team Employee KT Clearence
          </Subtitle2>
          {/* <div>
            <Caption1 className="text-gray-600">
              Manage Offboarding clearance for your team members
            </Caption1>
          </div> */}
        </div>
      </div>
      ...
      >
        <Card className="shadow-lg !rounded-xl border-0 overflow-hidden bg-white">
          <div className="overflow-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 flex flex-col gap-4">
            ...
            <Table sortable className="w-full">
              ...
              <TableBody>
                {paginatedExits.map((exit) => {
                  const currentStatus = exit.HeadKtStatus;
                  return (
                    <TableRow
                      key={exit.ID}
            ...
            {/* Drawer for Offboarding Details */}
            <Drawer
              position="end"
              open={isDrawerOpen}
...
const LeadKtClearenceDashboard = () => {
  return (
    <>
...
export default LeadKtClearenceDashboard;
