...
import { getEmployeeExitsByManager } from "../../Services/Offboarding";
...
const EmployeeExitsDashboard = () => {
  ...
  const loadExits = async () => {
    try {
      ...
      const response = await getEmployeeExitsByManager(
        currentUser.userID,
        accessToken
      );
  ...
  const calculateStats = (data: EmployeeExit[]) => {
    const stats = {
      total: data.length,
      pending: data.filter(
        (exit: EmployeeExit) => exit.HeadStatus === "Pending"
      ).length,
      locked: data.filter((exit: EmployeeExit) => exit.HeadStatus === "Locked")
        .length,
      completed: data.filter(
        (exit: EmployeeExit) => exit.HeadStatus === "Completed"
      ).length,
    };
  ...
  useEffect(() => {
    ...
    // Status filter
    if (statusFilter !== "all") {
      filtered = filtered.filter((exit) => exit.HeadStatus === statusFilter);
    }
  ...
  const handleViewDetails = (id: string) => {
    navigate(`/offboard/LeadClearance/${id}`);
  };
            ...
            <Table sortable className="w-full">
              ...
              <TableBody>
                {paginatedExits.map((exit) => {
                  const currentStatus = exit.HeadStatus;
                  return (
                    <TableRow
                      key={exit.ID}
...
export default LeadClearanceDashboard;
